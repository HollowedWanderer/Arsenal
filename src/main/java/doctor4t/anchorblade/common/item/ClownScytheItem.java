...
import doctor4t.clownscythe.common.damage.UseSpewingDamageSource;
import doctor4t.clownscythe.common.entity.BloodScytheEntity;
...
public class ClownScytheItem extends SwordItem {
    public ClownScytheItem(ToolMaterial material, float damage, float speed, Settings settings) {
    ...
    public boolean postHit(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        if (attacker instanceof PlayerEntity) {
            SoundEvent sound;
            spawnSweepParticles((PlayerEntity) attacker, ClownScythe.RED_SCYTHE_SWEEP_ATTACK_PARTICLE);
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        if (EnchantmentHelper.getEquipmentLevel(ClownScythe.SPEWING, user) > 0) {
            float f = 1.0f;
            ...
            if (!world.isClient) {
                ...
                user.damage(UseSpewingDamageSource.USE_SPEWING, 3f);
                ...
                if (world instanceof ServerWorld) {
                    ((ServerWorld) world).spawnParticles(ClownScythe.RED_SCYTHE_SWEEP_ATTACK_PARTICLE, user.getX() + d, user.getBodyY(0.5), user.getZ() + e, 0, d, 0.0, e, 0.0);
                }
            }
            world.playSound(null, user.getX(), user.getY(), user.getZ(), ClownScythe.ITEM_CLOWN_SCYTHE_SPEWING, SoundCategory.PLAYERS, 1.0f, 1.0f);
