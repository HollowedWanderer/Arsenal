...
public class ScytheItem extends MiningToolItem implements GUIHeldVaryingRenderItem, CustomHitParticleItem, CustomHitSoundItem, CustomColorItem {
        ...
        if (EnchantmentHelper.getEquipmentLevel(ArsenalEnchantments.SPEWING, user) > 0) {
            ...
            if (!world.isClient) {
                ...
                if (world instanceof ServerWorld serverWorld) {
                    ...
                    serverWorld.spawnParticles(ArsenalParticles.CLOWN_SCYTHE_SWEEP_ATTACK_PARTICLE, user.getX() + d, user.getBodyY(0.5) + pitch, user.getZ() + e, 0, d, 0.0, e, 0.0);
                }
            }
    ...
    public void spawnHitParticles(PlayerEntity player) {
        ...
        if (player.getWorld() instanceof ServerWorld serverWorld) {
            serverWorld.spawnParticles(ArsenalParticles.CLOWN_SCYTHE_SWEEP_ATTACK_PARTICLE, player.getX() + d0, player.getBodyY(0.5D), player.getZ() + d1, 0, d0, 0.0D, d1, 0.0D);
        }
    }
